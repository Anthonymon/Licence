...
if ((B.centre.y + B.rayon > HEIGHT) && (B.dy>0)) B.dy = -B.dy;
return B;
}
...
{
if ((B.centre.x <  WIDTH/2) && (B.centre.x+B.rayon >  WIDTH/2) && (B.dx>0) && (T.TV)) B.dx = -B.dx;
if ((B.centre.x >  WIDTH/2) && (B.centre.x-B.rayon <  WIDTH/2) && (B.dx<0) && (T.TV)) B.dx = -B.dx;
if ((B.centre.y < HEIGHT/2) && (B.centre.y+B.rayon > HEIGHT/2) && (B.dy>0) && (T.TH)) B.dy = -B.dy;
if ((B.centre.y > HEIGHT/2) && (B.centre.y-B.rayon < HEIGHT/2) && (B.dy<0) && (T.TH)) B.dy = -B.dy;
return B;
}
...
struct traits T;
T.TV = T.TH = 1;
T.H.x = T.B.x = WIDTH/2; T.H.y = HEIGHT; T.B.y = 0;
T.G.x = 0; T.D.x = WIDTH; T.G.y = T.D.y = HEIGHT/2;
return T;
}

// Change les traits de visible à invisible et vice-versa avec une proba de 0.001
...
{
POINT p;
char *s = "Fermer la fen�tre pour terminer";
